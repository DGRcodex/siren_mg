...
export default function ResultadoPage() {
  ...
  const formatCurrency = (amount: number, moneda: string) => {
    const locale = language === 'en' ? 'en-US' : language === 'he' ? 'he-IL' : 'es-CL';
  ...
  const getCriticidadColor = (nivel: string) => {
    // English
    if (nivel.toLowerCase() === 'critical') return 'text-destructive border-destructive';
    if (nivel.toLowerCase() === 'vulnerable') return 'text-accent border-accent';
    if (nivel.toLowerCase() === 'robust') return 'text-green-500 border-green-500';
    // Hebrew
    if (nivel === 'קריטי') return 'text-destructive border-destructive';
    if (nivel === 'פגיע') return 'text-accent border-accent';
    if (nivel === 'איתן') return 'text-green-500 border-green-500';
    // Spanish
    ...
    <div className="min-h-screen bg-background text-foreground py-12 px-4 sm:px-6 lg:px-8 print:bg-white print:text-black print:p-0">
      <div className="max-w-5xl mx-auto space-y-12 print:space-y-8">
        ...
        <div className="hidden print:flex justify-between items-center mb-8 border-b-2 border-black pb-6 mt-8">
          <div>
            <h1 className="text-3xl font-black text-black tracking-tighter">
              {t.navbar.brand}<span className="font-light">{t.navbar.brandSub}</span>
            </h1>
            <p className="text-sm font-semibold uppercase tracking-widest text-gray-500">{t.result.headerSub}</p>
          </div>
          <div className="text-end">
            <h2 className="text-xl font-bold">{t.result.reportTitle}</h2>
        ...
        <section className="bg-card border border-border rounded-3xl p-8 md:p-12 text-center shadow-2xl relative overflow-hidden print:border-black print:shadow-none print:bg-transparent print:rounded-none print:p-6 print:border-2">
          ...
          <h2 className="text-xl md:text-2xl text-muted-foreground mb-4 font-medium print:text-black print:font-bold">{t.result.lossEstimate}</h2>
          <div className="text-5xl md:text-7xl font-black text-transparent bg-clip-text bg-gradient-to-r from-destructive to-accent mb-6 flex flex-wrap items-center justify-center gap-4 print:text-black">
          ...
          <p className="text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto print:text-black">
            {t.result.lossDesc1} {resultado.nivelMadurezTexto}{t.result.lossDesc2}
          </p>
          <div className="mt-8 inline-block px-8 py-4 bg-background border border-border rounded-xl print:border-black print:bg-gray-100">
            <span className="font-semibold text-primary print:text-black">{t.result.maturityLvl} </span>
        ...
        <section className="print:break-inside-avoid">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="text-2xl font-bold print:text-black">{t.result.projTitle}</h3>
          </div>
          ...
          <div className="space-y-6">
            {resultado.proyectosEstrategicos.map((proj, idx) => (
              <div key={idx} className="bg-background border border-border rounded-xl p-6 relative overflow-hidden print:border-black print:bg-transparent">
                <div className="absolute ltr:left-0 rtl:right-0 top-0 bottom-0 w-1.5 bg-primary print:bg-black"></div>
                <div className="flex flex-col md:flex-row gap-6">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      <span className="bg-primary/20 text-primary text-xs font-bold px-2 py-1 rounded print:bg-gray-200 print:text-black">{t.result.initiative} {idx + 1}</span>
                    ...
                    {proj.kpiSugerido && (
                      <div className="inline-flex items-center gap-2 bg-secondary/50 border border-border px-4 py-2 rounded-lg text-sm print:border-black print:bg-gray-100">
                        ...
                        <span className="font-semibold text-muted-foreground print:text-black">{t.result.kpiToMeasure}</span>
                  ...
                  <div className="flex flex-row md:flex-col gap-2 min-w-[160px]">
                    <div className="flex-1 bg-card border border-border rounded-lg p-3 text-center print:border-gray-300">
                      <span className="block text-xs text-muted-foreground uppercase print:text-gray-600">{t.result.roi}</span>
                      <span className="font-bold text-accent print:text-black">{proj.impacto}</span>
                    </div>
                    <div className="flex-1 bg-card border border-border rounded-lg p-3 text-center print:border-gray-300">
                      <span className="block text-xs text-muted-foreground uppercase print:text-gray-600">{t.result.horizon}</span>
        ...
        <section className="print:break-before-page pt-8">
          <h3 className="text-2xl font-bold mb-6 print:text-black">{t.result.matrixTitle}</h3>
              ...
              <div key={idx} className="bg-card border border-border rounded-xl p-6 print:border-gray-400 print:bg-transparent">
                ...
                <div className="mb-4">
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-muted-foreground print:text-gray-600">{t.result.currentMaturity}</span>
                ...
                <div className="pt-4 border-t border-border flex justify-between items-end print:border-gray-300">
                  <div>
                    <span className="text-xs text-muted-foreground uppercase print:text-gray-600">{t.result.status}</span>
                    <p className={`font-bold text-sm ${getCriticidadColor(cat.nivelCriticidad)} print:text-black print:border-none`}>
                      {cat.nivelCriticidad}
                    </p>
                  </div>
                  <div className="text-end">
                    <span className="text-xs text-muted-foreground uppercase print:text-gray-600">{t.result.leak}</span>
        ...
        {resultado.analisisIA && (
          <section className="bg-card/30 border border-primary/30 rounded-2xl p-8 print:border-black print:bg-transparent print:mt-12">
            <div className="flex items-center gap-3 mb-4 border-b border-border pb-4 print:border-black">
              ...
              <h3 className="text-xl font-bold text-primary print:text-black">{t.result.aiAnalysisTitle}</h3>
            </div>
